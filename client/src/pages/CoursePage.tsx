...
import { Loader2, AlertCircle, Calendar, HardDrive, ChevronLeft, ChevronRight, Printer, Home, ChevronRight as ChevronRightIcon, Video, Music, FileText as FileTextIcon, Download } from 'lucide-react';
...
export const CoursePage: React.FC = () => {
  ...
  useEffect(() => {
    const renderAdvancedElements = async () => {
      ...
      // 3. Ensure KaTeX/Formula focus (already handled by rehype-katex)
    };
  ...
  const processHtml = (html: string) => {
    ...
    // 1. Transform relative image URLs
    ...
    let processed = html.replace(/<img([^>]*)src="([^"]+)"([^>]*)\/?>/g, (match, p1, src, p3) => {
      ...
      // Resolve relative path: remove ./ and handle ../
      let cleanSrc = src;
  ...
  const renderMedia = () => {
    ...
    const mime = content.metadata.mimeType;
      ...
      return (
        <div className="media-container mt-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
          <div className="flex items-center gap-3 mb-6 p-4 rounded-2xl bg-primary/10 border border-primary/20">
            ...
            <h3 className="text-xl font-bold m-0! text-white">Video: {content.name}</h3>
          </div>
          <div className="aspect-video rounded-3xl overflow-hidden border border-slate-800 shadow-2xl bg-slate-950">
            <video 
              controls 
              className="w-full h-full object-contain"
              poster={content.frontmatter?.poster ? `${assetsBaseUrl}/${content.frontmatter.poster}` : undefined}
            >
      ...
      return (
        <div className="media-container mt-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
          <div className="flex items-center gap-3 mb-6 p-4 rounded-2xl bg-secondary/10 border border-secondary/20">
            ...
            <h3 className="text-xl font-bold m-0! text-white">Audio: {content.name}</h3>
          </div>
        ...
        <div className="media-container mt-8 animate-in fade-in slide-in-from-bottom-4 duration-700 h-[80vh] flex flex-col">
          <div className="flex items-center justify-between gap-3 mb-6 p-4 rounded-2xl bg-accent/10 border border-accent/20">
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-xl font-bold m-0! text-white">Documento: {content.name}</h3>
            </div>
            <a 
              href={fileUrl} 
              download 
    ...
    return (
      <div className="p-12 rounded-3xl bg-slate-900 border border-slate-800 text-center flex flex-col items-center gap-6">
        ...
        <div>
          ...
          <p className="text-slate-400 mb-6">Este tipo de archivo ({content.metadata.mimeType}) no puede visualizarse directamente, pero puedes descargarlo.</p>
          <a 
            href={fileUrl} 
            download
        ...
        ">
          {content.html ? (
            <div 
              ...
              dangerouslySetInnerHTML={{ __html: processHtml(content.html) }} 
            />
          ) : renderMedia()}
        </div>
